...
function deduplicateByPhone(arr: { phone: string }[]): { phone: string }[] {
  const seen = new Set<string>();
  return arr.filter((item) => {
...
function deduplicateByWhatsAppPhone(
  arr: { url: string; phone: string }[],
...
async function checkBrokenLinks2(
  html: string,
  baseUrl: string,
  timeoutMs: number = 2000,
): Promise<{ url: string; status: string }[]> {
